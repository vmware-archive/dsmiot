...
function tseries(well_id, yr, mth, dt, hour, prob, div_id, data, feature, yaxis_label, line_color) {
    ...
    var line = d3.svg.line()
        .x(function(d, i) { return x(d.ts_utc + ndate_offset[i]*60*1000); })
    ...
    var ndate_offset = data.map(function(d){
                       return (new Date(d3.time.format.iso.parse(d.ts_utc).getTime())).getTimezoneOffset();
                   });
    ...
    x.domain(d3.extent(data, function(d, i) { return (d.ts_utc + ndate_offset[i]*60*1000); }));
    ...
    svg.append("text")
     ...
     .attr("cx", function (d, i) {
        return x(d.ts_utc + ndate_offset[i]*60*1000);
     })
     .attr("cy", function (d) {
       return y(d[feature]);
     })
...
function invokeTimeSeries(well_id, yr, mth, dt, hour, prob) {
    ...
    $.getJSON(
        ...
        function (data) {
            ...
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_depth', data.tseries, 'depth', "depth", "orange");
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_rpm', data.tseries, 'rpm', "rpm", "forestgreen");
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_rop', data.tseries, 'rop', "rate of penetration", "tomato");
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_wob', data.tseries, 'wob', "weight on bit", "mediumvioletred");
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_flowinrate', data.tseries, 'flow_in_rate', "flow-in Rate", "slateblue");
            tseries(well_id, yr, mth, dt, hour, prob, 'tseries_bitpos', data.tseries, 'bit_position', "bit position", "turquoise ");
        }
    );
}
